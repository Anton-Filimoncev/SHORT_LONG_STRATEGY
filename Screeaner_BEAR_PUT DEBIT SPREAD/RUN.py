...
from libs.get_debit_spread import debit_spread_run
...
if __name__ == "__main__":
    ...
    for i in range(len(active_stock_df['IV DIA year'])):
        try:
            ...
            active_stock_df['IV DIA year'].iloc[i] = np.nan
            pass
    ...
    # Для тикеров с 1-2 диапазоном собираем медвежий дебет спред
    RR, needed_strike_sell, needed_strike_buy, proba, expected_return = debit_spread_run(
        active_stock_df, stock_yahoo, tick_list, poll_num, RISK_RATE
    )

    active_stock_df["Strike Short"] = needed_strike_sell
    active_stock_df["Strike Long"] = needed_strike_buy
    active_stock_df["RR"] = RR
    active_stock_df["Proba"] = proba
