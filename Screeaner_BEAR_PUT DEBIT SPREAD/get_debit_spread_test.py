...
from libs.popoption.PutDebitSpread import putDebitSpread
...
def get_data_and_calc_debit_spread(start_df_native, stock_yahoo_short, RISK_RATE):
    KEY = "ckZsUXdiMTZEZVQ3a25TVEFtMm9SeURsQ1RQdk5yWERHS0RXaWNpWVJ2cz0"
    # try:
    # start_df, stock_yahoo_short, RISK_RATE = pool_input
    for i in range(len(start_df_native)):
        start_df = start_df_native.iloc[i]
        tick = start_df['Symbol']
        hv = float(start_df['HV 100'])
        print(tick)
    ...
    return RR, expected_return


active_stock_df = pd.read_excel("SCREENER_RESULT.xlsx")[:2]
tick_list = active_stock_df["Symbol"].values.tolist()
stock_yahoo_short = yf.download(tick_list, group_by="ticker")
RISK_RATE = 4
# active_stock_df.iloc[i]
RR, needed_strike_sell, needed_strike_buy, proba, expected_return = get_data_and_calc_debit_spread(active_stock_df, stock_yahoo_short, RISK_RATE)
