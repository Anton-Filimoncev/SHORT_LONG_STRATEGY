...
def get_data_and_calc_pop(pool_input):
    ...
    try:
        ...
        if int(start_df['IV DIA year']) >= 2:
            ...
            rpop_50 = chains["50% RETURN"] * np.array(monte_carlo_proba_50)
            rpop_50 = rpop_50.max()
...
def sell_put_run(active_stock_df, stock_yahoo, tick_list, poll_num):
    print('---------------------------')
    print('------------- Getting SELL PUT ... --------------')
    print('---------------------------')

    with Pool(poll_num) as p:
        sell_put_result = p.map(get_data_and_calc_pop, [(active_stock_df.iloc[i], stock_yahoo) for i in range(len(active_stock_df))])

    return sell_put_result
