...
def get_data_and_calc_strangl(pool_input):
    ...
    try:
        ...
        if int(start_df['IV DIA year']) == 2 or int(start_df['IV DIA year']) == 3:
            ...
            needed_exp_date = \
                exp_date_df[exp_date_df['Days_to_exp'] == days_to_exp]['expirations'].reset_index(
                    drop=True).iloc[0].date()
            ...
            needed_put = \
                put_df[put_df['Difference'] == put_df['Difference'].max()].reset_index(drop=True).iloc[0]
            ...
            # put
            otm_value_put = np.where(needed_put['strike'] > current_price, 0,
            ...
            # call
            otm_value_call = np.where(needed_call['strike'] < current_price, 0,
                                      needed_call['strike'] - current_price)
            needed_call['Margin CALL'] = np.where(
                (needed_call['strike'] * 0.1) > (current_price * 0.2 - otm_value_call),
                (needed_call['strike'] * 0.1), (current_price * 0.2 - otm_value_call))

            total_margin = np.where((needed_call['Margin CALL']) > (needed_put['Margin PUT']),
...
def strangle_run(active_stock_df, stock_yahoo, tick_list, poll_num):
    ...
    with Pool(poll_num) as p:
        strangle_out = p.map(get_data_and_calc_strangl,
                         [(active_stock_df.iloc[i], stock_yahoo) for i in range(len(active_stock_df))])
