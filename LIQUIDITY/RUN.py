...
if __name__ == "__main__":
    poll_num = 3  # Количество потоков
    # Загружаем датафрейм с компаниями
    active_stock_df = pd.read_csv(
        "active_stock/most-active-stock-options-10-03-2023.csv"
    )[:-1]
    ...
    # regime_list, relative_regime_list = get_company_signals_run(stock_yahoo, tick_list, poll_num)  # pooled
    #
    ...
    # IV_percentile, IV_Regime, IV_Median, IV, HV_20, HV_50, HV_100, HV_Regime = get_ib_run(tick_list, poll_num)
    ...
    # sell_put_result = sell_put_run(active_stock_df, stock_yahoo, tick_list, poll_num)
    # time.sleep(3)

    active_stock_df = pd.read_excel("Active_Stock_Screaner.xlsx")[:20]

    # sell_call_result = sell_call_run(active_stock_df, stock_yahoo, tick_list, poll_num)

    # active_stock_df['SELL_PUT'] = sell_put_result
    # active_stock_df['SELL_CALL'] = sell_call_result

    # # Для тикеров с 2-3 диа собираем стренглы
    # strangle_data = strangle_run(active_stock_df, stock_yahoo, tick_list, poll_num)  # pooled
    ...
    # bear_calendar_data = bear_calendar_run(active_stock_df, stock_yahoo, tick_list, poll_num)
    # active_stock_df['BEAR_CALENDAR'] = bear_calendar_data

    # active_stock_df.to_excel('Active_Stock_Screaner.xlsx', index=False)
