...
def get_data_and_calc_itm_calendar(pool_input):
    ...
    try:
        ...
        exp_date_df['expirations'] = pd.to_datetime(exp_date_df['expirations'])
        exp_date_df['Days_to_exp'] = (exp_date_df['expirations'] - datetime.datetime.now()).dt.days
        days_to_exp_short = nearest_equal_abs(exp_date_df['Days_to_exp'], 7)
        needed_exp_dateshort = \
        exp_date_df[exp_date_df['Days_to_exp'] == days_to_exp_short]['expirations'].reset_index(drop=True).iloc[0].date()


        # ----------- Chains -----------------
        url = f"https://api.marketdata.app/v1/options/chain/{tick}/?expiration={needed_exp_dateshort}&side=call&token={KEY}"
        ...
        needed_delta = nearest_equal_abs(chains_short['delta'], 0.3)
        # needed_strike_sell = nearest_equal_abs(chains_short['delta'], 0.3)
        needed_short = chains_short[chains_short['delta'] == needed_delta].reset_index(drop=True).iloc[0]
        # needed_short = all_needed_exp_df_sell[all_needed_exp_df_sell['iv'] == all_needed_exp_df_sell['iv'].max()].iloc[0]
        needed_strike_sell = needed_short['strike']
        print('needed_strike_sell', needed_strike_sell)


        current_price = needed_short['underlyingPrice']
        ...
        exp_date_df['expirations'] = pd.to_datetime(exp_date_df['expirations'])
        exp_date_df['Days_to_exp'] = (exp_date_df['expirations'] - datetime.datetime.now()).dt.days
        days_to_exp_long = nearest_equal_abs(exp_date_df['Days_to_exp'], 70)
        ...
        needed_strike_buy = nearest_equal_abs(chains_long['strike'], current_price)

        needed_long = chains_long[chains_long['strike'] == needed_strike_buy].reset_index(drop=True).iloc[0]


        print('needed_strike_buy', needed_strike_buy)
        print(needed_long)
        ...
        print(err)
        caledar_call_score, needed_strike_sell, needed_strike_buy, proba_30, expected_return = np.nan, np.nan, np.nan, np.nan, np.nan
        pass

    return caledar_call_score, needed_strike_sell, needed_strike_buy, proba_30, expected_return


def otm_calendar_run(active_stock_df, stock_yahoo, tick_list, poll_num, RISK_RATE):
    print('---------------------------')
    ...
    caledar_call_score, needed_strike_sell,  needed_strike_buy, proba_30, expected_return = zip(*itm_calendar_out)
