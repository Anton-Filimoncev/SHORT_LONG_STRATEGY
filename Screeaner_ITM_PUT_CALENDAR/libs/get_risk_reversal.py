...
def calculate_probability_call(
    hist_data, nb_simulations, days, lower_bound, upper_bound, target_price, side
):
    ...
    if side == "C":
        touch_target_price = (price_list >= float(target_price)).any(
            axis=0
        ).sum() / nb_simulations
    if side == "P":
        touch_target_price = (price_list <= float(target_price)).any(
...
def calculate_probability_put(
    hist_data, nb_simulations, days, lower_bound, upper_bound, below_price_list, side
):
    # Fetch historical market data
    # hist_data = yf.download(ticker, start='2000-01-01')
...
def get_data_and_calc_long(pool_input):
    ...
    try:
        ...
        if int(start_df["IV DIA year"]) == 1:
            ...
            price_up_options_list_call = option_price_calc(
                price_up,
                needed_chains_call["strike"].tolist(),
                days_to_exp,
                needed_chains_call["iv"].tolist(),
                "C",
            )[0]
            ...
            otm_value = np.where(
                needed_chains_put["strike"] > current_price,
                0,
                current_price - needed_chains_put["strike"],
            )
            margin = np.where(
                (needed_chains_put["strike"] * 0.1) > (current_price * 0.2 - otm_value),
                (needed_chains_put["strike"] * 0.1),
            ...
            pot = calculate_probability_call(
                stock_yahoo_long[tick],
                10000,
                close_exp_date,
                0,
                0,  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            poe = calculate_probability_put(
                stock_yahoo_long[tick],
                10000,
                days_to_exp,
                0,
                0,  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                needed_chains_put["strike"],
                "P",
            )[
                0
            ]  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
#                 chains["Days_to_exp"] = (
#                     chains["expiration"] - datetime.datetime.now()
#                 ).dt.days
...
#
#             needed_strike_sell = nearest_equal_abs(
#                 all_needed_exp_df_sell["strike"], current_price - exp_move
#             )
#
#             all_needed_exp_df_sell = all_needed_exp_df_sell[
#                 all_needed_exp_df_sell["strike"] == needed_strike_sell
#             ]
#             needed_short = all_needed_exp_df_sell[
#                 all_needed_exp_df_sell["iv"] == all_needed_exp_df_sell["iv"].max()
#             ].iloc[0]
#
#             # print('needed_short')
#             # print(needed_short)
#
#             exp_dates_long = exp_date_df[
#                 exp_date_df["Days_to_exp"] >= needed_short["Days_to_exp"] + 30
#             ]
...
#
#             all_needed_exp_df_buy = all_needed_exp_df_buy[
#                 all_needed_exp_df_buy["strike"] == needed_strike_sell
#             ]
#             needed_long = all_needed_exp_df_buy[
#                 all_needed_exp_df_buy["iv"] == all_needed_exp_df_buy["iv"].min()
#             ].iloc[0]
#
#             # print('needed_long')
#             # print(needed_long)
...
#
#             caledar_put_score = (
#                 (expected_return / (debet * 100)) / needed_short["Days_to_exp"] * 365
#             )
#
#             print("caledar_put_score", caledar_put_score)
#
#             expected_price = current_price - exp_move
#
#         else:
...
#         expected_price = "EMPTY"
#
#     return [caledar_put_score, expected_price]


def risk_reversal_run(active_stock_df, stock_yahoo, tick_list, poll_num):
    print("---------------------------")
    print("------------- Getting Risk Reversal... --------------")
    print("---------------------------")

    with Pool(poll_num) as p:
        risk_reversal_out = p.map(
            get_data_and_calc_long,
            [
    ...
    return np.array(risk_reversal_out)
