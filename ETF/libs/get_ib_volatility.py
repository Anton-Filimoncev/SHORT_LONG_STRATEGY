...
def market_stage_ticker(pool_input):
    try:
        ...
        unsup = mix.GaussianMixture(
            n_components=4, covariance_type="spherical", n_init=100, random_state=42
        )
        ...
        Regimes = (
            pd.DataFrame(regime, columns=["Regime"], index=df.index)
            .join(df, how="inner")
        ...
        lisusss = [
            one_period_price_mean,
            two_period_price_mean,
            three_period_price_mean,
            four_period_price_mean,
        ]
        lisusss_sort = sorted(lisusss)

        period_set = {
            0: lisusss_sort.index(lisusss[0]) + 1,
        ...
        # ================================================     hist vol ======================
        unsup_hist = mix.GaussianMixture(
            n_components=4, covariance_type="spherical", n_init=100, random_state=42
        )
        ...
        Regimes_hist = (
            pd.DataFrame(regime, columns=["Regime"], index=df_hist.index)
        ...
        one_period_price_mean_hist = Regimes_hist[Regimes_hist["Regime"] == 0][
            "close"
        ].max()
        two_period_price_mean_hist = Regimes_hist[Regimes_hist["Regime"] == 1][
            "close"
        ].max()
        three_period_price_mean_hist = Regimes_hist[Regimes_hist["Regime"] == 2][
            "close"
        ].max()
        four_period_price_mean_hist = Regimes_hist[Regimes_hist["Regime"] == 3][
            "close"
        ].max()

        lisusss_hist = [
            one_period_price_mean_hist,
            two_period_price_mean_hist,
            three_period_price_mean_hist,
            four_period_price_mean_hist,
        ]
        lisusss_sort_hist = sorted(lisusss_hist)

        period_set_hist = {
            0: lisusss_sort_hist.index(lisusss_hist[0]) + 1,
        ...
        HV_200 = Regimes_hist["close"].rolling(window=200).mean().iloc[-1]
...
def get_df_chains(ticker_contract, limit_date_min, limit_date_max, tick, rights, ib):
    ...
    chains = ib.reqSecDefOptParams(
        ticker_contract.symbol, "", ticker_contract.secType, ticker_contract.conId
    )
    ...
    contracts = [
        Option(tick, expiration, strike, right, "SMART", tradingClass=tick)
        for right in rights
...
def get_avalible_exp_dates(ticker, limit_date_expiration_min):
    with client.connect():  # Make any requests for data inside this block. Requests made outside this block wont run.
        ...
        all_exp_date = exp_date_data[
            exp_date_data > limit_date_expiration_min
        ...
        for exp_date in all_exp_date:
            strike_list.append(
                client.get_strikes(ticker, exp_date).astype("float").values.tolist()
            )
...
def get_ib_run(tick_list, poll_num):
    ...
    with Pool(poll_num) as p:
        pool_out = p.map(
            market_stage_ticker,
            [(id_num, tick) for id_num, tick in enumerate(tick_list)],
        )
    ...
    HV_200 = np.array([*HV_200])
    HV_200 = np.reshape(HV_200, len(HV_200))

    return IV_percentile, IV_Regime, IV_Median, IV, HV_20, HV_50, HV_100, HV_200, HV_Regime
