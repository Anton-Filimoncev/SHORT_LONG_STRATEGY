...
if __name__ == "__main__":
    ...
    days_to_exp = nearest_equal_abs(exp_date_df["Days_to_exp"], 200)
    needed_exp_date = (
    ...
    put_df["ATM_strike_volatility"] = put_df[put_df["strike"] == atm_put_strike][
        "iv"
    ].values[0]
    ...
    needed_call_delta = nearest_equal_abs(call_df["delta"].dropna(), abs(put_delta))
    needed_call = (
        call_df[call_df["delta"] == needed_call_delta].reset_index(drop=True).iloc[0]
    )
    ...
    otm_value_put = np.where(
        needed_put["strike"] > current_price, 0, current_price - needed_put["strike"]
    )
    ...
    otm_value_call = np.where(
        needed_call["strike"] < current_price, 0, needed_call["strike"] - current_price
    )
    ...
    print("strangle_pop_50: ", strangle_pop_50)
